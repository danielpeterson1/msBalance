...
During each asssessment, subjects stood with arms folded across the chest, eyes open, with their feet at a fixed heel-to-heel distance of 10 cm.
...
For this preliminary analysis, we considered only the activity of tibialis anterior of the right leg during forward perturbations.
...
```{r rawEmg}
library(ggplot2)
p = ggplot(taR, aes(x = Time, y = V, group = Code)) + geom_line() + facet_grid(`Study Group` ~ `Perturbation Direction`)
p
```
