...
captionRawEmg = figureNumbers(name = "rawEmg", caption = "EMG traces stratified by perturbation direction and study group.")
```
...
taR = left_join(taR, trialData, by = "Code") %>% 
...
taR = taR %>% filter(Participant %in% includedParticipants)
participants = participants %>% filter(Participant %in% includedParticipants)

taR = taR %>% mutate(Group = factor(`Study Group`, levels = c("Control", "MS"))) %>% select(-`Study Group`) %>% 
  mutate(`Perturbation Direction` = factor(`Perturbation Direction`, levels = c("Forward", "Backward")))

# calculate max activity for normalization purposes. this has already been filtered, etc. so it is probably ok just to normalize to maximum values. note that when we do this, it really appears to favor some narrow stuff. it's probably better to average over replicates of each direction before calculating maximum values.
taRNorm = taR %>% group_by(Participant) %>% summarize(Vm = max(V, na.rm = TRUE)) %>% ungroup()
taR = taR %>% left_join(taRNorm, by = "Participant") %>% mutate(Vn = V/Vm)
...
Recorded EMG traces are presented in `r figureNumbers("rawEmg", display = "cite")`.

***
```{r rawEMG, out.width = "100%", fig.align = "left", fig.cap = captionRawEmg}
library(ggplot2)
library(cowplot)
p = ggplot(taR, aes(x = Time, y = Vn, group = Code)) + geom_line() + facet_grid(Group ~ `Perturbation Direction`) + theme_minimal_grid()
p
```
***
